...
from database_manager import DatabaseManager, Post, Reply
...
class FeedProcessor:
        ...
        with self.db_manager.get_session() as session:
            for idx, feed_item in enumerate(reversed(feed), 1):
                try:
                    ...
                    logging.error(f"Error processing feed item: {str(e)}", exc_info=True)
                    session.rollback()
                    continue
    ...
    def _process_feed_item(self, session: Session, feed_item: Dict, idx: int, total: int):
        ...
        if replies:
            ...
            for reply in replies:
                try:
                    ...
                    logging.error(f"Error processing reply for post {post_id}: {str(e)}", exc_info=True)
                    continue
